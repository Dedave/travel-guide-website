...
export default function PlanTripModal() {
  ...
  return (
    <>
      ...
      {/* Backdrop */}
      <div
        className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center sm:p-4 bg-black/80 backdrop-blur-sm wl-backdrop"
        onClick={(e) => e.target === e.currentTarget && close()}
      >
        {/* Modal — bottom sheet on mobile, centered card on sm+ */}
        <div
          className={`bg-[#0f0f1a] border border-white/10 w-full sm:max-w-lg sm:rounded-3xl rounded-t-3xl overflow-hidden shadow-2xl max-h-[92dvh] flex flex-col ${
        ...
        >
          ...
          <div className="overflow-y-auto flex-1">
            ...
            >
              ...
              {/* Badge */}
              <div className="relative inline-flex items-center gap-2 bg-yellow-400/10 border border-yellow-400/30 text-yellow-400 text-[10px] font-bold tracking-[0.2em] uppercase px-3 py-1.5 rounded-full mb-3 sm:mb-4">
                <Compass className="h-3 w-3" />
                Plan Your Trip
              </div>

              {/* Headline */}
              <h2 className="relative font-display text-[22px] sm:text-[26px] font-black text-white leading-tight mb-2">
              ...
              <p className="relative text-white/50 text-sm leading-relaxed mb-4 sm:mb-5">
              ...
              <div className="relative flex flex-col xs:flex-row gap-2">
                <input
                  type="text"
                  value={dest}
                  ...
                  className="flex-1 bg-white/5 border border-white/15 rounded-xl text-white text-sm px-4 py-3 outline-none placeholder:text-white/30 focus:border-yellow-400/60 transition-colors min-w-0"
                />
                <button
                  onClick={handleGo}
                  className={`inline-flex items-center justify-center gap-1.5 font-black text-xs px-5 py-3 rounded-xl transition-all whitespace-nowrap w-full xs:w-auto ${
                    updated
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 gap-2 p-3 sm:p-4">
              {cards.map((card: Card) => (
                <a 
                  key={card.label}
                  href={card.href}
                  ...
                  className="wl-card-hover group bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] hover:border-white/[0.18] rounded-2xl p-3 sm:p-4 block no-underline"
                >
                  <div className="text-xl sm:text-2xl mb-1.5 sm:mb-2">{card.icon}</div>
                  <div className="text-white text-xs sm:text-sm font-bold mb-0.5 sm:mb-1">
                    {card.label}
                  </div>
                  <div className="text-white/40 text-[10px] sm:text-[10.5px] leading-tight mb-1.5 sm:mb-2">
                    {card.desc}
                  </div>
          ...
          <div className="border-t border-white/[0.07] px-4 sm:px-5 py-3 flex items-center justify-between gap-4 shrink-0">
